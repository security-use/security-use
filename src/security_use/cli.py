...
def _ecosystem_from_purl(purl: str) -> str:
    """Extract ecosystem name from a Package URL (PURL).

    Maps PURL types to OSV ecosystem names.
    """
    purl_to_ecosystem = {
        "pypi": "PyPI",
        "npm": "npm",
    ...
    if purl.startswith("pkg:"):
        ...
        return purl_to_ecosystem.get(purl_type, "PyPI")
    return "PyPI"
...
def sbom_enrich(sbom_file: str, output: Optional[str]) -> None:
    ...
    # Detect SBOM format (CycloneDX vs SPDX)
    is_cyclonedx = sbom_data.get("bomFormat") == "CycloneDX"
    is_spdx = "spdxVersion" in sbom_data

    if not is_cyclonedx and not is_spdx:
        console.print("[red]Error: Unrecognized SBOM format. Supports CycloneDX and SPDX JSON.[/red]")
        sys.exit(1)
    ...
    try:
        if is_cyclonedx:
            ...
            for component in components:
                ...
                for vuln in vulns:
                    vuln_count += 1
                    bom_ref = component.get("bom-ref", component.get("purl", name))
                        ...
                        "ratings": [
                            {
                                ...
                                **({"score": vuln.cvss_score} if vuln.cvss_score else {}),
                                "method": "CVSSv3",
                            }
                        ],
                        "description": vuln.title,
                        "recommendation": f"Upgrade to version {vuln.fixed_version}" if vuln.fixed_version else "No fix available yet",
                        "affects": [{"ref": bom_ref}],
                    })

            sbom_data["vulnerabilities"] = vulnerabilities_list

        elif is_spdx:
            packages = sbom_data.get("packages", [])
            ext_doc_refs = sbom_data.get("externalDocumentRefs", [])

            for package in packages:
                name = package.get("name", "")
                version = package.get("versionInfo", "")
                if not name or not version or version == "0.0.0":
                    continue

                # Determine ecosystem from external refs
                purl = ""
                ...
                vulns = osv.query_package(name, version, ecosystem)
                if vulns:
                    # Add vulnerability annotations to the package
                    ...
                    for vuln in vulns:
                        ...
                        package["annotations"].append({
                            ...
                            "annotator": "Tool: security-use-0.2.9",
                            "annotationDate": datetime.datetime.now(datetime.timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ"),
                            "comment": f"Vulnerability {vuln.id}: {vuln.title} (severity: {vuln.severity.value})"
                            + (f" - Fix: upgrade to {vuln.fixed_version}" if vuln.fixed_version else ""),
                        })
    finally:
        osv.close()

    # Write enriched SBOM
    enriched_content = json.dumps(sbom_data, indent=2)

    if output:
        Path(output).write_text(enriched_content, encoding="utf-8")
        console.print(f"[green]Enriched SBOM written to {output}[/green]")
    else:
        click.echo(enriched_content)

    console.print(f"[green]Found {vuln_count} vulnerabilities across components[/green]")
