...
class ProjectDetector:
    ...
    def detect(self) -> ProjectInfo:
        ...
        # Check for Docker
        info.has_dockerfile = (self.root / "Dockerfile").exists() or (self.root / "docker-compose.yml").exists()
        
        # Check for pre-commit
        info.has_pre_commit = (self.root / ".pre-commit-config.yaml").exists()
        
        # Check for existing security-use config
        info.has_security_use_config = (self.root / ".security-use.yaml").exists() or (self.root / "security-use.yaml").exists()
    ...
    def _scan_python_files(self) -> list[AppFile]:
        ...
        entry_points = ['main.py', 'app.py', 'application.py', 'server.py', 'api.py', 'wsgi.py', 'asgi.py']
...
'''
    ...
    def inject(self, app_file: AppFile, dry_run: bool = False) -> tuple[bool, str]:
        ...
        app_pattern = self.FASTAPI_MIDDLEWARE if app_file.framework == Framework.FASTAPI else self.FLASK_MIDDLEWARE
    ...
    def initialize(
        ...
        if inject_middleware and info.primary_app:
            ...
            if app.has_middleware:
                ...
                results['middleware']['message'] = msg if not success else f"Injected middleware into {app.path.name}"
